...
Examples:
  ...
  $0 --no-pcre2 -c             # Minimal build, config only
...
if command -v rg &> /dev/null; then
    version=$(rg --version | head -1 | cut -d' ' -f2 2>/dev/null || echo "unknown")
fi

# Check if we can use cached binary
if is_cached "ripgrep" "$BUILD_TYPE" "$version"; then
    log "Found cached ripgrep build, using cached version..."
    if get_cached_binary "ripgrep" "$BUILD_TYPE" "$version" "rg"; then
...
if ! is_cached "ripgrep" "$BUILD_TYPE" "$version" || ! get_cached_binary "ripgrep" "$BUILD_TYPE" "$version" "rg" &>/dev/null; then
    ...
    version=$(rg --version | head -1 | cut -d' ' -f2 2>/dev/null || echo "unknown")
    cache_build "ripgrep" "$BUILD_TYPE" "$version" "$HOME/.cargo/bin/rg"
fi
...
if command -v rg &> /dev/null; then
    ...
    log "You can now use the 'rg' command"
    echo
