...
log "Starting just $MODE process for Debian Linux"
log "Build type: $BUILD_TYPE"
log "Mode: $MODE"


# Handle just source code
if [[ -d "$JUST_DIR" ]]; then
    log "Found existing just directory: $JUST_DIR"
    
    # Check if it's a git repository
    if [[ -d "$JUST_DIR/.git" ]]; then
        ...
        if [[ "$CURRENT_ORIGIN" == "$JUST_REPO" ]]; then
            log "Repository origin matches expected just repository"
            ...
            cd ..
            rm -rf "$JUST_DIR"
            log "Cloning just repository..."
            git clone "$JUST_REPO" "$JUST_DIR" || error "Failed to clone just repository"
            cd "$JUST_DIR"
            success "just repository cloned successfully"
        fi
        cd ..
    else
...
# Change to just directory
cd "$JUST_DIR"

# Verify we're in the correct directory
if [[ ! -f "Cargo.toml" ]]; then
    error "Invalid just source directory - missing Cargo.toml"
fi
...
log "Configuring just with $BUILD_TYPE settings..."
...
# Build just
log "Building just (this may take a while)..."
...
if [[ ! -f "$BUILD_DIR/just" ]]; then
    error "Build completed but just executable not found in $BUILD_DIR"
fi
...
if [[ "$MODE" == "build" ]]; then
    ...
    log "Build output: $(pwd)/$BUILD_DIR/just"
    exit 0
fi
...
if command -v just &> /dev/null; then
    version=$(just --version | head -1 | cut -d' ' -f2 2>/dev/null || echo "unknown")
fi

# Check if we can use cached binary
if is_cached "just" "$BUILD_TYPE" "$version"; then
    log "Found cached just build, using cached version..."
    if get_cached_binary "just" "$BUILD_TYPE" "$version" "just"; then
        ...
        success "just installed from cache successfully"
...
if ! is_cached "just" "$BUILD_TYPE" "$version" || ! get_cached_binary "just" "$BUILD_TYPE" "$version" "just" &>/dev/null; then
    ...
    sudo ln -sf "$HOME/.cargo/bin/just" /usr/local/bin/just || warning "Failed to create just symlink"
    success "Symlink created for just command"
    
    # Cache the new build - get fresh version after installation
    version=$(just --version | head -1 | cut -d' ' -f2 2>/dev/null || echo "unknown")
    cache_build "just" "$BUILD_TYPE" "$version" "$HOME/.cargo/bin/just"
fi
...
hash -r
if command -v just &> /dev/null; then
    success "just installation verified!"
    echo
    log "just version information:"
    just --version
    echo
    log "Build type: $BUILD_TYPE"
    ...
    log "You can now use the 'just' command"
    echo
    ...
    error "just installation verification failed - try restarting your shell or run 'source ~/.bashrc'"
fi