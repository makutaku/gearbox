...
Examples:
  ...
  $0 -r --run-tests    # Full build with tests
...
if command -v fd &> /dev/null; then
    version=$(fd --version | head -1 | cut -d' ' -f2 2>/dev/null || echo "unknown")
fi

# Check if we can use cached binary
if is_cached "fd" "$BUILD_TYPE" "$version"; then
    log "Found cached fd build, using cached version..."
    if get_cached_binary "fd" "$BUILD_TYPE" "$version" "fd"; then
...
if ! is_cached "fd" "$BUILD_TYPE" "$version" || ! get_cached_binary "fd" "$BUILD_TYPE" "$version" "fd" &>/dev/null; then
    ...
    success "Symlink created for fd command"
    
    # Cache the new build - get fresh version after installation
    version=$(fd --version | head -1 | cut -d' ' -f2 2>/dev/null || echo "unknown")
    cache_build "fd" "$BUILD_TYPE" "$version" "$HOME/.cargo/bin/fd"
fi
...
if command -v fd &> /dev/null; then
    ...
    log "You can now use the 'fd' command"
    echo
    log "Usage examples:"
    log "  fd pattern                    # Search for files matching pattern"
    log "  fd -t f pattern               # Search only files (not directories)"
    log "  fd -e rs                      # Search for .rs files"
    log "  fd -H pattern                 # Include hidden files"
